...
进程终止后， 会保持zombie， 直到被父进程回收reaped，内核才把子进程的退出状态传递给父进程，抛弃子进程。父进程终止了的话， 内核会安排init进程成为他的孤儿进程的养父， init 的pid为1， 是在系统启动时由内核创建的， 不会终止， 是所有进程的祖先。 不过， 长时间运行的程序， 如shell或者服务器， 总是应该回收zombie。
